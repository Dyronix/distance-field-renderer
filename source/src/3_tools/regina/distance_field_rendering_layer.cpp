...
namespace regina
{
    ...
    namespace deferred_rendering
    {
        ...
        MeshNameMap MESH_NAME_MAP = {
            {MeshType::BUNNY, "Bunny"_sid},   {MeshType::CUBE, "Cube"_sid},   {MeshType::CYLINDER, "Cylinder"_sid}, {MeshType::MONKEY, "Monkey"_sid},
            {MeshType::SPHERE, "Sphere"_sid}, {MeshType::TORUS, "Torus"_sid}, {MeshType::DRAGON, "Dragon"_sid},     {MeshType::TIGER, "Tiger"_sid},
        };
        MeshScaleMap MESH_SCALE_MAP{
            {MeshType::BUNNY, {10.0, 10.0, 10.0}}, {MeshType::CUBE, {0.5, 0.5, 0.5}},  {MeshType::CYLINDER, {0.5, 0.5, 0.5}},  {MeshType::MONKEY, {0.5, 0.5, 0.5}},
            {MeshType::SPHERE, {0.5, 0.5, 0.5}},   {MeshType::TORUS, {0.5, 0.5, 0.5}}, {MeshType::DRAGON, {0.02, 0.02, 0.02}}, {MeshType::TIGER, {1.5, 1.5, 1.5}},
        };
        ...
        rex::DistanceEvaluationsPassOptions create_distance_evaluation_pass_options()
        {
            ...
            options.pass_name = deferred_rendering::DISTANCEEVALUATIONSPASS_NAME;
        ...
        rex::DeferredLightPassOptions create_deferred_light_pass_options()
        {
            ...
            options.g_position_buffer = {deferred_rendering::DISTANCEEVALUATIONSPASS_NAME, 0};
            options.g_normal_buffer = {deferred_rendering::DISTANCEEVALUATIONSPASS_NAME, 1};
            options.g_albedo_spec_buffer = {deferred_rendering::DISTANCEEVALUATIONSPASS_NAME, 2};
        ...
        rex::DeferredLightVisualizationPassOptions create_deferred_light_visualization_pass_options()
        {
            ...
            options.depth_pass_name = deferred_rendering::DISTANCEEVALUATIONSPASS_NAME;
    ...
    void DistanceFieldRenderingLayer::setup_scene_renderer()
    {
        ...
        auto distance_eval = create_distance_evaluation_pass(deferred_rendering::create_distance_evaluation_pass_options());
    ...
    //-------------------------------------------------------------------------
    void DistanceFieldRenderingLayer::setup_geometry()
    {
        R_PROFILE_FUNCTION();

        const int32 MAX_OBJECTS = 9;

        std::array<rex::vec3, MAX_OBJECTS> object_positions = {rex::vec3(-3.0, -0.0, -3.0), rex::vec3(0.0, -0.0, -3.0), rex::vec3(3.0, -0.0, -3.0), rex::vec3(-3.0, -0.0, 0.0), rex::vec3(0.0, -0.0, 0.0),
                                                               rex::vec3(3.0, -0.0, 0.0),   rex::vec3(-3.0, -0.0, 3.0), rex::vec3(0.0, -0.0, 3.0),  rex::vec3(3.0, -0.0, 3.0)};
    ...
    //-------------------------------------------------------------------------
    std::unique_ptr<rex::SceneRenderPass> DistanceFieldRenderingLayer::create_pre_depth_pass(const rex::PreDepthPassOptions& options) const
    {
        R_PROFILE_FUNCTION();
