...
if __name__ == "__main__":
    ...
    if is_library:
        ...
        template +=     "add_executable(" + project_name + " " + library_type + " ${" + project_name + "_LIBS_INC} ${" + project_name + "_LIBS_SRC})\n"
    ...
    template +=     "IF(MSVC)\n"
    template +=     "\tset_target_properties(" + project_name + " PROPERTIES VS_DEBUGGER_WORKING_DIRECTORY                ${OUTPUT_BINDIR})        \n"
    template +=     "\tset_target_properties(" + project_name + " PROPERTIES ARCHIVE_OUTPUT_DIRECTORY                     ${OUTPUT_BINDIR})        \n"
    template +=     "\tset_target_properties(" + project_name + " PROPERTIES LIBRARY_OUTPUT_DIRECTORY                     ${OUTPUT_BINDIR})        \n"
    template +=     "\tset_target_properties(" + project_name + " PROPERTIES RUNTIME_OUTPUT_DIRECTORY                     ${OUTPUT_BINDIR})        \n"
    template +=     "\n\n"
    ...
    if not is_library:
        if platform.system().lower() == "windows":
            ...
            template +=     "    POST_BUILD\n"
            template +=     "    WORKING_DIRECTORY ${SCRIPTS_DIR}\\${LOWER_PLATFORM_TARGET}\n"
            template +=     "    COMMAND PowerShell -File cmake-post-build.ps1\n"
            template +=     ")\n"
        else:
